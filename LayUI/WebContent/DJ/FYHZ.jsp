...
<body>
	<div class="layui-fluid">
		<div class="layui-card">
					...
					<div class="layui-form" lay-filter="freshform">
						<table class="layui-table" lay-filter="test" id="test_table">
							...
							<tbody>
								<tr>
									<td></td>
									<td><select name="XMMC_DATA" class="proNameSel" lay-filter="projectNameSel">
								...
								<tr class="addlists">
									...
									<td>合计</td>
									<td colspan=4></td>
									<td><input type="text" name="ZJ" class="layui-input"></td>
								</tr>

								<tr>
									<td rowspan="2"></td>
									<td rowspan="2">费用总计</td>
									<td colspan="2">电汇金额总计</td>
									<td colspan="2">（除叉车仓储外其他费用）除叉车仓储外其他费用</td>
									<td><input type="text" name="" class="layui-input"></td>
								</tr>

								<tr>
									<td colspan="2">现场缴纳金额总计</td>
									<td colspan="2">（叉车及仓储费用）</td>
									<td><input type="text" name="" class="layui-input"></td>
								</tr>
							</tbody>
						</table>
			...
			<script>
				layui.use([ 'form', 'laydate', 'layer', 'table', ],
								function() {
												...
												.ajax({
													...
													success : function(d) {
														...
														for (var i = 0; i < d.length; i++) {
															xmName = d[i].XMMC;
															xmID = d[i].ID;
															xmmc += '<option value="' + xmName + '" id="'+xmID+'">'
																	+ xmName
																	+ '</option>';
														}
														if (isadd) {
									...
									var xmms_select = function(isadd, elemId) {
										...
										if (isadd) {
											...
											xmmc = $(
													"[name=XMMC_DATA] option:selected")
													.val();
												...
												.ajax({
													...
													success : function(d) {
														...
														for (var i = 0; i < d.length; i++) {
															...
															var msID = d[i].ID;
															var fy = d[i].FY;
															xmms += '<option value="' + msName + '" id="'+msID+'">'
																	+ msName
																	+ ','
																	+ fy
																	+ '</option>';
									...
									//项目描述监听事件
									form.on('select(projectDescSel)', function(
											index) {
										var sums = 0;
										var dj = parseInt(index.elem.innerText
												.split(',')[1]);
										$('[name=DJ0]').val(dj);

										if ($('[name=SL0]').val() != '') {
											var sl = Number($('[name=SL0]')
													.val())
											var xj = dj * sl;
