											...
											<blockquote class="layui-elem-quote layui-quote-nm"
												...
												<div class="layui-upload-list" id="sfzDiv">
													<button type="button" class="layui-btn"
														style="display: block; margin: 0 auto" id="sfzz">加盖公章的个人身份证扫描件</button>
												</div>
											</blockquote>
										</div>
									</div>
										...
										<div class="layui-upload">
											<blockquote class="layui-elem-quote layui-quote-nm"
												...
												<div class="layui-upload-list" id="yyzzDiv">
													<button type="button" class="layui-btn"
														style="display: block; margin: 0 auto" id="yyzzz">营业执照</button>
												</div>
											</blockquote>
										</div>
									</div>
										...
										<div class="layui-upload">
											<blockquote class="layui-elem-quote layui-quote-nm"
												...
												<div class="layui-upload-list" id="frsfzDiv">
													<button type="button" class="layui-btn"
														style="display: block; margin: 0 auto" id="frsf">上传法人身份证</button>
												</div>
											</blockquote>
										</div>
									</div>
...
html body { -
		...
		layui.use([ "jquery", "upload", "form", "layer", "element" ],function() {
							...
							upload.render({
										...
										/* before : function(obj) {
											...
											obj.preview(function(index, file,result) {
												$("#sfz").removeAttr("style");
											});
										}, */
										before: function (obj) {
							                //预读本地文件示例，不支持ie8
							                obj.preview(function (index, file, result) {
							                    $("#sfzz").removeAttr("style");
							                });
							            }, 
										done : function(res) {										
											$('#sfzz').before("<input id='sfzsmj' class='ant-input ant-input-lg bitian' name='SFZSMJ' style='display: none' value='"+res.data.src+"'/>");
										/* 	 $('#sfzz').before("<img src='"+ res.data.src +"' id='sfzImg' alt=''  style='margin-bottom: 5px;' class='layui-upload-img'>"); 	 */
											
											  $('#sfzz').before("<div class='file-iteme' style='width: 92px;height: 92px;display:inline-block;margin-right: 10px;'><img src='"+ res.data.src +"' id='sfzImg' alt=''  style='margin-bottom: 5px;position:relative;' class='layui-upload-img uploader-list'>  <span class='info' style='position:absolute;z-index:10;display:none'><a href='"+res.data.src+"' target='_blank'><image src='statics/login/prew.png'></image></a></span><span class='info' style='position:absolute;z-index:10;display:none' onclick='del(this)'><image src='statics/login/del.png'></image></span></div>");
										}
									});
							/*营业执照*/
							upload.render({
										...
										before: function (obj) {
							                //预读本地文件示例，不支持ie8
							                obj.preview(function (index, file, result) {
							                    $("#yyzzz").removeAttr("style");
							                });
							            }, 
										done : function(res) {
											$('#yyzzz').before("<input id='yyzz' class='ant-input ant-input-lg bitian' name='YYZZ' style='display: none' value='"+res.data.src+"'/>");
											/* $('#yyzzz').before("<img src='"+ res.data.src +"' id='yyzzImg' alt=''  style='margin-bottom: 5px;' class='layui-upload-img'>"); */
											$('#yyzzz').before("<div class='file-iteme' style='width: 92px;height: 92px;display:inline-block;margin-right: 10px;'><img src='"+ res.data.src +"' id='yyzzImg' alt=''  style='margin-bottom: 5px;position:relative;' class='layui-upload-img uploader-list'>  <span class='info' style='position:absolute;z-index:10;display:none'><a href='"+res.data.src+"' target='_blank'><image src='statics/login/prew.png'></image></a></span><span class='info' style='position:absolute;z-index:10;display:none' onclick='del(this)'><image src='statics/login/del.png'></image></span></div>");
							...
							upload.render({
										...
										before: function (obj) {
							                //预读本地文件示例，不支持ie8
							                obj.preview(function (index, file, result) {
							                    $("#frsf").removeAttr("style");
							                });
							            }, 
										done : function(res) {
											$('#frsf').before("<input id='frsfzz' class='ant-input ant-input-lg bitian' name='FRSFZZ' style='display: none' value='"+res.data.src+"'/>");
											/* $('#frsf').before("<img src='"+ res.data.src +"' id='frsfzImg' alt=''  style='margin-bottom: 5px;' class='layui-upload-img'>"); */
											$('#frsf').before("<div class='file-iteme' style='width: 92px;height: 92px;display:inline-block;margin-right: 10px;'><img src='"+ res.data.src +"' id='frsfzz' alt=''  style='margin-bottom: 5px;position:relative;' class='layui-upload-img uploader-list'>  <span class='info' style='position:absolute;z-index:10;display:none'><a href='"+res.data.src+"' target='_blank'><image src='statics/login/prew.png'></image></a></span><span class='info' style='position:absolute;z-index:10;display:none' onclick='del(this)'><image src='statics/login/del.png'></image></span></div>");
	...
	function Validate() {
		...
		var sfzz = document.getElementById("sfzsmj"); 
		if(!sfzz){
			alert("请上传身份证扫描件");
			$("#sfzz").focus();
			return false;
		}
		var yyzzz = document.getElementById("yyzz"); 
		if(!yyzzz){
			alert("请上传营业执照");
			$("#yyzzDiv").focus();
			return false;
		}
		var frsf = document.getElementById("frsfz"); 
		if(!frsf){
			alert("请上传法人身份证照");
			$("#frsf").focus();
			return false;
