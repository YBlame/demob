					...
					        //请求成功时处理
							...
							for (var i = 0; i < con.length; i++) {
								...
								if (con[i].formtypes=='radio'||con[i].formtypes=='select'||con[i].formtypes=='checkbox') {
									...
									input+="&nbsp;&nbsp;&nbsp;&nbsp;<div class='layui-input-inline' style='width:"+con[i].width+"px;"+display+"'><select name='"+con[i].zdm+"'  id='"+con[i].zdm+"' >"+option+"</select></div>";
									option="";
								}
...
layui.use([ 'laydate', 'laypage', 'layer', 'table', 'carousel', 'upload',
				...
				function() {
					...
					element.on('tab(demo)', function(data) {
						layer.tips('切换了 ' + data.index + '：' + this.innerHTML,
								this, {
									tips : 1
								});
					});
					...
					// 监听行工具事件
					table.on('tool(test)', function(obj) { // 注：tool
